...
//después cambiar traerpersonas por funcion que traiga personas y aplique filtros existentes 
//antes de traer personas validar localstorage, si existe la lista local la cargo y generotabla
//si no existe traigo las personasy genero la tabla
window.addEventListener('load', iniciaPag);

function iniciaPag(){
    traerPersonas();
    if(!(localStorage.getItem('listaClicks'))){
        let lista = [1,2,3,1,2,1,1,1,4,5,5,4,4];
        localStorage.setItem('listaClicks', JSON.stringify(lista));
    }
    if((localStorage.getItem('listaClicks'))){
        let clicks = JSON.parse(localStorage.getItem('listaClicks'));
        console.log(clicks);
        let uno = 0;
        let dos = 0;
        let tres = 0;
        ...
        var ctx = document.getElementById("myChart");
                  var myChart = new Chart(ctx, {
                      type: 'bar',
                      data: {
                          labels: ["anuncio 1", "anuncio 2", "anuncio 3", "anuncio 4", "anuncio 5"],
                          datasets: [{
                              label: 'Groups',
...
function actualizaLocal(nuevaLista){
    if(!(localStorage.getItem('listaAnuncios'))){
        localStorage.setItem('listaAnuncios', JSON.stringify(nuevaLista));
    }
}
...
function traerAnuncios() {
    ...
    if((localStorage.getItem('listaAnuncios'))){
        let lista = JSON.parse(localStorage.getItem('listaAnuncios'));
        return lista;
    }
}
...
function cargarCheck(){
    //chekear chekbox
    let listaLocal = [];
    if(!(localStorage.getItem('listaSeleccion'))){
        //genero local de chekbox
        listaLocal = obtenerChek();
        console.log(listaLocal);
        localStorage.setItem('listaSeleccion', JSON.stringify(listaLocal));
    }else{
        //obtiene local de chekbox y los carga
        listaLocal = obtenerLocal();
        chekeoBox(listaLocal);
    }
}

function obtenerChek(){
    let listaLocal = [];
    console.log(divFiltros);
    if(divFiltros.hasChildNodes())
    {
        let hijos = divFiltros.childNodes;
        console.log(hijos);
        hijos.forEach(hijo => {
            if ((hijo.checked)) {
                // console.log(hijo.id);
                listaLocal.push(hijo.id);
            }
        });
    }
    console.log(listaLocal);
...
function obtenerLocal(){
    if(localStorage.getItem('listaSeleccion')){
        //obtengo local de chekbox
        return JSON.parse(localStorage.getItem('listaSeleccion'));
    }
}

function pisoLocal(listaLocal){
    if(localStorage.getItem('listaSeleccion')){
        //obtengo local de chekbox
        localStorage.removeItem('listaSeleccion');
        localStorage.setItem('listaSeleccion', JSON.stringify(listaLocal));
    }
}

function chekeoBox(listaLocal){
    if(divFiltros.hasChildNodes())
    {
        let hijos = divFiltros.childNodes;
        hijos.forEach(hijo => {
            hijo.checked = false;
            listaLocal.forEach(unChek => {
                if(hijo.id == unChek){
...
main.addEventListener('click', (e) =>{
    ...
    let btnC = document.querySelector('#btnCancela');
        ...
        if (e.target == divTabla || e.target == main || e.target == art2 || e.target == artf || e.target == contTabla || e.target == divroww) {
...
function filterAnu(trxSel){
    ...
    let listaPersonas = traerAnuncios();
...
function generoTabla(lista)
{
    ...
    if(lista.length > 0){
        ...
        tabla.setAttribute('class', 'table table-primary table-bordered');
        ...
        let tablaOff = document.createElement('img');
        tablaOff.setAttribute('src', './img/grillaoff.gif');
        tablaOff.setAttribute('class', 'imgTitel');
        let mensajeT = document.createElement('h2');
        let msg = document.createTextNode('no existen datos para mostrar');
        mensajeT.appendChild(msg);
        divTabla.appendChild(tablaOff)
        divTabla.appendChild(mensajeT)
...
function funcionesTabla()
{
    ...
    // else {
        ...
        btn1.setAttribute('class', 'btn btn-primary miPrimary2 bg-danger');
        btn2.setAttribute('class', 'btn btn-primary miPrimary2 bg-succes');
        btn3.setAttribute('class', 'btn btn-primary miPrimary2 bg-warning');
    // }
}
...
function funcionesFiltros()
{
    ...
    //piso filtros checkeado local

    let lista = obtenerChek();
    pisoLocal(lista);
    // // console.log(listaPersonas);
    // console.log('aplicando filtros');
    // let xhr = new XMLHttpRequest();
